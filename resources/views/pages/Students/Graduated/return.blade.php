        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{route('Graduates.update','test')}}" method="post" autocomplete="off">
                    ...
                    <h5 style="font-family: 'Cairo', sans-serif;">{{trans('Students_trans.student_back_graduate')}}</h5>
                    <input type="text" readonly value="{{$student->Name}}" class="form-control">

                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">{{trans('Students_trans.Close')}}</button>
                        <button  class="btn btn-danger">{{trans('Students_trans.student_back')}}</button>
                    </div>
                </form>
            </div>
        </div>
